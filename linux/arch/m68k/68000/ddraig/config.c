...
void __init ddraig_init_IRQ(void)
{
}

void ddraig_sched_init(irq_handler_t handler)
{
	printk(KERN_INFO "Setting up Ddraig timer hardware\n");

	setup_irq(IRQ_NUM_TIMER, &ddraig_timer_irq);
...
{
	printk(KERN_INFO "Y Ddraig support by Stephen Moody\n");
...
int __init ddraig_platform_init(void)
{
